...
class CameraJointControl:
    ...
    def set_joint_velocity(self, speeds):
        """
        Wrapper for setting joint velocities using the xArm API.
        
        Args:
            speeds (list/array): Joint velocities for each joint
            duration (float): Duration for velocity motion, -1 for continuous motion
        
        Returns:
            bool: True if successful, False otherwise
        """
        ret = self.arm.vc_set_joint_velocity(speeds, is_radian=True, is_sync=True, duration=1/15)
        return True
    ...
    def convert_action_to_angles_and_gripper(self, action_step):
        ...
        return joint_velocities.tolist(), gripper_command
        ...
        while True:
            ...
            if action_chunk is not None:
                ...
                for idx in range(min(self.open_loop_horizon, len(action_chunk))):
                    ...
                    self.set_joint_velocity(new_angles)
